...
from dava.models import AnnealedVAE
...
def main():
    ...
    parser.add_argument("--iteration_threshold", type=int, default=100000 * 64)
    ...
    vae = AnnealedVAE(z_dim=z_dim, num_channels=num_channels,
                      c_max=c_max, gamma=gamma, iteration_threshold=iteration_threshold,
    ...
    train_annealed_vae(vae, dataset, device, store_path=store_path, batch_size=batch_size, num_steps=num_steps,
                       learning_rate=learning_rate, max_grad_norm=max_grad_norm)


def train_annealed_vae(vae: AnnealedVAE, data: npt.ArrayLike, device, store_path: str, batch_size=32,
                       num_steps=5, learning_rate=1e-4, max_grad_norm=1.):
    print("Start AnnealedVAE training")
